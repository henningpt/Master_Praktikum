...
Liegt kein Schnittpunkt mit einer Achse vor, so wird der Millerindex $0$ gewählt.
...
\begin{align}
f=\int_{\mathrm{Hülle}} \symup{e}^{-\symup{i}\pi\Delta\phi} \rho\left( \vec{r}\right) \symup{d}^3 r = \int_{\mathrm{Hülle}} \symup{e}^{-2\pi\symup{i}\left(\vec{k}-\vec{k}_0\right)} \rho\left( \vec{r}\right) \symup{d}^3
\end{align}
angesehen werden.
